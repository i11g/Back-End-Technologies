...
namespace TownApplication.IntegrationTests
{
    public class TownControllerIntegrationTests
    {
        ...
        public void AddTown_ValidInput_ShouldAddTown()
        {
            ...
            Assert.Equal(result.Name, "Sofia");
        }
        ...
        public void AddTown_InvalidName_ShouldThrowArgumentException(string invalidName)
        {
            ...
            var message=Assert.Throws<ArgumentException>(()=>_controller.AddTown(invalidName, populationNumber));
        ...
        public void AddTown_DuplicateTownName_DoesNotAddDuplicateTown()
        {
            ...
            var validTwon = "Plovdiv";
            var population = 100000;
            var population1 = 200000;

            // Add a town initially to set up the duplicate scenario.
            _controller.AddTown(validTwon, population);
           

            // Act: Try to add the same town again.
            _controller.AddTown(validTwon, population1);
            ...
            var totalTowns = _controller.ListTowns().Count;
            Assert.Equal(totalTowns, 1);
            var towninDB=_controller.ListTowns().FirstOrDefault();
            Assert.Equal(towninDB.Name, validTwon);
            Assert.Equal(towninDB.Population, population);
        ...
        public void UpdateTown_ShouldUpdatePopulation()
        {
           ...
           var townInDB =_controller.GetTownByName(townName);
            _controller.UpdateTown(townInDB.Id, updatedPopulation);
            ...
            var townInB = _controller.GetTownByName(townName);
            Assert.NotNull(townInB);
            Assert.Equal(townInB.Population, updatedPopulation);
        }
        ...
        public void DeleteTown_ShouldDeleteTown()
        {
            ...
            var result=_controller.GetTownByName(townNmae);
            Assert.Null(result);
        }
        ...
        public void ListTowns_ShouldReturnTowns()
        {
            ...
            List <string> townNames = new List<string> () { "Varna", "Sofia", "plovdiv" };
            foreach(var townName in townNames)
            {
                _controller.AddTown(townName, townName.Length * 10);
            }
            ...
            foreach(var townName in townNames)
            {
                var existingTown=listOfTowns.FirstOrDefault(t=>t.Name==townName);
                Assert.NotNull(existingTown);
                Assert.Equal(existingTown.Population, townName.Length * 10);
            }
        }
    }
}
