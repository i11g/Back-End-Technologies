    ...
    public class TownControllerIntegrationTests
    {
        ...
        public void AddTown_ValidInput_ShouldAddTown()
        {
            ...
            // Call the AddTown method on the _controller with the arranged data.
                _controller.AddTown(validName,validPopulation);
            ...
            Assert.NotNull(validPopulation);
            var addedTown=_controller.GetTownByName(validName);
            Assert.NotNull(addedTown);
            Assert.Equal(validName, addedTown.Name);
        ...
        public void AddTown_InvalidName_ShouldThrowArgumentException(string invalidName)
        {
            ...
            var result=Assert.Throws<ArgumentException>(()=>_controller.AddTown(invalidName,populationNumber));
        ...
        public void AddTown_InvalidPopulation_ShouldThrowArgumentException(int invalidPopulation)
        {
            ...
            // Replace the placeholder value with actual valid data.
            var validTownName = "Pleven";

            // Act: Execute the action to be tested.
            // Wrap the call to AddTown in an Action delegate to use with Assert.Throws.
            //Assert.Throws<ArgumentException>(()=>_controller.AddTown(validTownName,invalidPopulation));
            ...
            var result=Assert.Throws<ArgumentException>(action);
            Assert.Equal("Population must be a positive number.", result.Message);
        }
        ...
        public void AddTown_DuplicateTownName_DoesNotAddDuplicateTown()
        {
            ...
            var townName = "Sofia";
            var townPopulation = 10000000;


            // Add a town initially to set up the duplicate scenario.
            _controller.AddTown(townName, townPopulation);

            // Act: Try to add the same town again.
            _controller.AddTown(townName, townPopulation + 100);
            ...
            var totalNumber = _controller.ListTowns();
            Assert.Single(totalNumber);
            var result = totalNumber[0];
            Assert.Equal(townName, result.Name);
        }

        [Fact]
        public void UpdateTownMinusPopulation_ShouldUpdatePopulation()
        {
        ...
        public void ListTowns_ShouldReturnTowns()
        {
            ...
            var towns = new List<string> { "Sofia", "Plovdiv", "Pleven" }; 
            ...
            foreach (var town in towns)
            {
                //var wziatingTown = allTowns.Any(x => x.Name == town);
                //Assert.True(wziatingTown);
                var existingTown = allTowns.FirstOrDefault(x => x.Name == town);
                Assert.NotNull(existingTown);
                Assert.Equal(existingTown.Population, town.Length + 1000);
